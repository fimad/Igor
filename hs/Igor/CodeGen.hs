...
jump indexLabel reason = do
    ...
    case reason' of
        ...
        (Because reason a b)    -> do
            ...
            calculateJump (G.Jump reason) indexLabel
...
calculateJump :: (Integer -> G.Gadget) -> Label -> Predicate ()
    ...
    where
        ...
        replaceJumpHolders state@CodeGenState{..} predIndex byteOffset j@(Left (JumpHolder {..}))
            | predIndex == labelToIndex state jumpTarget   = do
                let jumpGarbageSize         = if (predIndex-jumpIndex) <= 0 then jumpLength else 0
